...
class WeighingMachineService {
  ...
  static Future<List<WeighingMachineModel>> getUserMachines() async {
    try {
      ...
      // ── DEBUG: show token prefix and URL ──────────────────────────────────
      final tokenPreview = headers['Authorization'] ?? 'NULL';
      debugPrint('[WM] GET ${AppConstants.baseUrl}${AppConstants.getUserMachines}');
      debugPrint('[WM] Auth: ${tokenPreview.length > 40 ? tokenPreview.substring(0, 40) + "..." : tokenPreview}');
      ...
      // ── DEBUG: show full status + body so we can see exactly what came back
      ...
      if (res.statusCode == 200) {
        ...
        // Handle both { machines: [] } and plain array []
        List<dynamic> list;
        if (decoded is List) {
          list = decoded;
        } else if (decoded is Map) {
          // Try common key names the backend might use
          list = (decoded['machines']
              ?? decoded['data']
        ...
        return list
            .map((e) => WeighingMachineModel.fromJson(e as Map<String, dynamic>))
            .toList();
      }
      ...
      // ── DEBUG: full error + stack so nothing is silently swallowed ────────
  ...
  static Future<List<WeighingMachineModel>> getMachinesByEsp(String espId) async {
    try {
      final headers = await ApiService.authHeaders();
      final res = await http
          .get(
        Uri.parse('${AppConstants.baseUrl}${AppConstants.getMachinesByEsp(espId)}'),
        headers: headers,
      )
          .timeout(const Duration(seconds: 10));

      debugPrint('[WM] getMachinesByEsp($espId) → ${res.statusCode}: ${res.body}');

      if (res.statusCode == 200) {
        final decoded = jsonDecode(res.body);
        List<dynamic> list = decoded is List
            ? decoded
            : ((decoded as Map)['machines'] ?? decoded['data'] ?? []) as List<dynamic>;
        return list
            .map((e) => WeighingMachineModel.fromJson(e as Map<String, dynamic>))
            .toList();
      }
  ...
  static Future<bool> registerMany(List<Map<String, dynamic>> sensors) async {
    try {
      final headers = await ApiService.authHeaders();
      debugPrint('[WM] registerMany: ${sensors.length} sensor(s) → ${jsonEncode({'sensors': sensors})}');
      final res = await http
          .post(
        Uri.parse('${AppConstants.baseUrl}${AppConstants.registerManyMachines}'),
        headers: headers,
  ...
  static Future<WeighingMachineModel?> registerOne({
    ...
    try {
      ...
      final res = await http
        ...
        headers: headers,
        body: jsonEncode({
          'espId': espId, 'slot': slot, 'name': name,
          'location': location, 'category': category,
          'threshold': threshold, 'capacity': capacity, 'unit': unit,
        }),
      )
          .timeout(const Duration(seconds: 10));
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        return WeighingMachineModel.fromJson(machineData as Map<String, dynamic>);
      }
      return null;
    } catch (e) {
  ...
  static Future<bool> updateMachine(
      String sensorId, {
        ...
        double? threshold, double? capacity, String? unit,
      }) async {
    try {
      final headers = await ApiService.authHeaders();
      ...
      final res = await http
          .patch(
        Uri.parse('${AppConstants.baseUrl}${AppConstants.updateMachine(sensorId)}'),
        headers: headers,
  ...
  static Future<bool> removeMachine(String sensorId) async {
    try {
      ...
      final res = await http
          .delete(
        Uri.parse('${AppConstants.baseUrl}${AppConstants.deleteMachine(sensorId)}'),
        headers: headers,
      )
          .timeout(const Duration(seconds: 10));
